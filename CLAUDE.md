...
- **The test suite passes** (`npm test` for unit specs, `npm run test:e2e` for the integration suite). The unit specs cover `UsersService`/`BusinessService` (incl. the auth/security fixes), `RefreshTokenService` (reuse-detection, against live Redis), `CacheService`, the `assertOwnership` util, `app.controller`, and the `AnalyticsService`/`DiscountService`/`DeliveryService` business logic; the e2e suite (`test/smoke.e2e-spec.ts`) boots the full `AppModule` against a real PostgreSQL instance and exercises auth (incl. refresh rotation + reuse-detection, business refresh, FTS search), RBAC, analytics, the commerce flow, and security/commerce regressions. The old auto-generated controller specs were stale (wrong symbols, missing DTO fields) and were removed/replaced. `test:e2e` is pinned to `maxWorkers: 1` so the two app-booting suites don't race on TypeORM `synchronize`. There are still no specs for the `admin`/`upload`/`custom-order`/`category`/`mail` modules — see `IMPROVEMENT_PLAN.md`.
